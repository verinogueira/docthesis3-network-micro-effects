%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Network analysis for IRL and KOR
% Codes adapted from:
% Vasco M Carvalho
...
% This file is identical to WIOD_Network.m, except for:
% numeber of industries reduced from 56 to 48
% last section (exporting the data)
...
for m=1:M
    cent_KB_down(:,m) = (eye(48)-GammaX(:,:,m))\sum(GammaX(:,:,m),2); 
end
...
% n = number of sectors (48 industries)
% 
v = zeros(N,N,M); 
cent_Carv_down = zeros(N,M); 

for m=1:M
    v(:,:,m)=(0.5/48)*inv(eye(48)-0.5*W(:,:,m));
...
for m=1:M
    cent_KB_up(:,m) = (eye(48)-GammaXT(:,:,m))\sum(GammaXT(:,:,m),2); 
end
...
for m=1:M
    v1(:,:,m)=(0.5/48)*inv(eye(48)-0.5*W1(:,:,m));
...
% outdegrees
sigmao = sum(Sigma,2);
sigmao = (permute(sigmao, [2 1 3])); 
sigmao = reshape(sigmao,[48,2]);
...
distance_w=distance_w';
        ...
        filename = 'WIOD_Network_IRL_KOR.xls';
...
for i=1:length(M_countries) 
        IND = table(industry,ecc(:,i),gammao(:,i), cent_Carv_down(:,i), cent_Carv_up(:,i), cent_eig_down(:,i), cent_eig_up(:,i), cent_KB_down(:,i), cent_KB_up(:,i), cent_Sigma_down(:,i),cent_Sigma_up(:,i),sodw(:,i));
        filepath = 'C:\Users\Veri\OneDrive - University of Essex\RESEARCH\PUBLISHING\Chapter3\Codes\Stata\Input'; 
        filename = sprintf('Industries_Network_%s_48.xls',M_countries(i));
...
% 
% for i=1:length(M_countries) 
% y = [mu(:,i)';mu_test(:,i)';cent_KB_down(:,i)';KB_test(:,i)'];
% bar(industry,y','EdgeColor','none','BarWidth',1)
% legend('mu','mu_test','cent_KB_down','KB_test')
% set(findall(gcf,'-property','FontSize'),'FontSize',12)
% title('Test centralities','FontSize',14), xlabel('Industries'), ylabel('Index')
%          x0=50, y0=50, width=900, height=600;
%          set(gcf,'position',[x0,y0,width,height])
% xticks([1:48]), xtickangle(90)
% %saveas(gcf,'test_IRL_KOR.png')
% end
% 
% % mu and KB are not similar when using the model. gamma_out and beta also
% % not similar and not uniform like 1/N
% % not sure why cross country plots (data) ended up similar
%  
% 
% for i=1:length(M_countries) 
% y = [gammao(:,i)';beta(:,i)'];
...
% a=[gammao(:,i)];
% b=[beta(:,i)];
% [AX,H1,H2] =plotyy([1:48],a, [1:48],b, 'bar', 'bar');
